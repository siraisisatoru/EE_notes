...
void rainbowUpdate() {
    cRGB localPixel;
    // for (rainbowUpdateIndex = 0; rainbowUpdateIndex < 256 * 5; rainbowUpdateIndex++) { // 5 cycles of all colors on wheel
    if (rainbowUpdateIndex < 256) {
        for (uint16_t i = 0; i < LED_COUNT; i++) {
            localPixel = Wheel(((i * 256 / LED_COUNT) + rainbowUpdateIndex) & 255);
            // if (localBrightness) {
            //     localPixel.r = (localPixel.r * localBrightness) >> 8;
            //     localPixel.g = (localPixel.g * localBrightness) >> 8;
            //     localPixel.b = (localPixel.b * localBrightness) >> 8;
            // }
            pixels[i] = localPixel;
            RGBtoByteArr();
    ...
    if (LED_COUNT == 6) {
        if (theaterChaseDelay == delayTheshold - 1) {
            for (uint8_t i = 0; i < LED_COUNT; i++) {
                // if (localBrightness) {
                //     pixels[i].r = (moreColor.r * localBrightness) >> 8;
                //     pixels[i].g = (moreColor.g * localBrightness) >> 8;
                //     pixels[i].b = (moreColor.b * localBrightness) >> 8;
                // } else {
